...
from ..ml_models.predict_delay import predict_delay as predict_task_delay
...
def _to_frontend(task: models.Task) -> dict[str, Any]:
    return {
        ...
        "delay_probability": getattr(task, "delay_probability", None),
        "delay_risk": getattr(task, "delay_risk", None),
    }
...
def create_task(
    ...
    # ML delay prediction
    prediction = predict_task_delay({
        "priority": task.priority,
        "complexity_score": task.complexity_score,
        "due_date": task.due_date,
    })
    task.delay_probability = prediction.get("probability")
    task.delay_risk = prediction.get("risk_level")
